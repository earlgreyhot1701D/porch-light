...
- `adapters/` (CivicEngage AgendaCenter / Ventura parsing, pure deterministic), `agents/` (the two genuine agents — extractor and watcher — one module each plus their tools; §38), `verify/` (the six-check rewrite verifier, pure functions), `search/` (bridge table, lexical, vector, fusion), `web/` (public site + read-only API), `pipeline/` (the **deterministic hunter** — scheduler entry, run lock, worklist, spend ledger, run log; the hunter is NOT an agent, §38), `db/` (schema, migrations), `tests/` (including fixtures built from real Ventura documents and one deliberately poisoned PDF).
