...
| **Aug 27–29** | **2 — Ingestion + hunter** | Hash change detection, run lock with TTL + heartbeat, idempotency, run log, good-citizen fetch posture, **deterministic hunter** (a scheduled Lambda, no model, no agent loop — §38). | Second run on unchanged data does nothing and says so. Crashed-and-restarted run double-writes nothing. |
